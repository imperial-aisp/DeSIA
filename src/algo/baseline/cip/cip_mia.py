...
class CIPAttack(SolverBaseAttack):
    ...
    def prepare_script(self, input_constraints: dict, tmp_target_dataset, filename: str):        
        """
        ...
        """
        _, aux_dataset, _, domain_config, _ = self.model_inputs[filename]
        row_constraint = len(tmp_target_dataset)
        ...
        if self.args.init_solver is True:
            ...
            aux_test_size_ratio = len(aux_dataset) / len(tmp_target_dataset)
    ...
    def _attack(self, _input_args):
        ...
        filename, indexed_row, unique_records = _input_args
        released_aggregates, _, target_dataset, domain_config, _ = self.model_inputs[filename]
        ...
        scripts = self.prepare_script(input_constraints, tmp_target_dataset, filename)
        feasible_solutions = []
        ...
        # Save different reconstructed dataset into different dataframes
        for solution_idx, feasible_solution in enumerate(feasible_solutions):
            ...
            df_census_raw_solution = pd.DataFrame(dict_solution)
            df_census_solution = df_census_raw_solution.groupby(by = tmp_target_dataset.columns.tolist()).size().reset_index(name="count").sort_values(by = ["count"], ascending = False)
            df_census_solution.to_csv(os.path.join(self.save_dir.format(filename), "solution_by_unique_rows-{}-{}.csv".format(unique_record_idx, solution_idx)), index = False)
        # Save the target dataframe as one
        df_census_target = tmp_target_dataset.groupby(by = tmp_target_dataset.columns.tolist()).size().reset_index(name="count").sort_values(by = ["count"], ascending = False)
    ...
    def _evaluate(self, _input_args):
        ...
        unique_record = tuple(val for val in unique_record.items())
        for solution_idx in range(self.args.num_solutions):
            df_census_solution = pd.read_csv(os.path.join(self.save_dir.format(filename), "solution_by_unique_rows-{}-{}.csv".format(unique_record_idx, solution_idx)))
            census_solution = {tuple(row[:-1].tolist()): row[-1] for _, row in df_census_solution.iterrows()}
            if unique_record in census_solution:
                mia_guess = 1
            else:
                mia_guess = 0
            total_mia_guess.append(mia_guess)
            file_log.append("Target record is a member in solution-{}: {}".format(solution_idx, True if mia_guess == 1 else False))
        
        eval_time = time.time() - eval_start_time    
        with open(os.path.join(self.save_dir.format(filename), "logs-{}.txt".format(unique_record_idx)), "a") as fp:
            fp.write("\nEvaluation time (seconds): {}\n".format(eval_time))
            fp.write("\nThere are {}/{} solutions with a membership of target record {}\n".format(sum(total_mia_guess), self.args.num_solutions, unique_record))
            fp.write("\n".join(file_log))
