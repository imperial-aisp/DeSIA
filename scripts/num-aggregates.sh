...
do
        for num_aggregates in 28 56 113 225 421 
        do
                ...
                python $python_script --dataset ppmf --filenames ${BLOCKS[@]} --seed $seed --num-aggregates $num_aggregates --test-size $test_size --chosen-gpus 0 --multi-cpus 5 --note $note \
                        rap
                python $python_script --dataset ppmf --filenames ${BLOCKS[@]} --seed $seed --num-aggregates $num_aggregates --test-size $test_size --chosen-gpus 0 --multi-cpus 5 --note $note \
                        rap --warm-start
