...
class StepOneAttack(BaseAttack):
    ...
    def _step_one_generate_script(self, input_constraints: dict, indexed_row: pd.Series, filename: str):
        ...
        _step_one_script += "\n{}target_quasi_id_potential_record_indexs = {}".format(self.row_indent, target_quasi_id_potential_record_indexs)
        ...
        # Add the secondary constraint, re-run solver
        assert self.args.mode == "aia"
        _step_one_script += "\n{}{}for index in target_quasi_id_potential_record_indexs:".format(self.row_indent, self.row_indent)
        _step_one_script += "\n{}{}{}if round(V[index].Xn) == 1:".format(self.row_indent, self.row_indent, self.row_indent)
        _step_one_script += "\n{}{}{}{}model.addConstr(V[index] == 0)".format(self.row_indent, self.row_indent, self.row_indent, self.row_indent)
        ...
        if self.args.noise_budget is None:
            # For clean aggregates, infeasible solver instances are treated as fatal.
            _step_one_script += "\n{}elif model.Status == 3:".format(self.row_indent)
            _step_one_script += "\n{}{}exit(1)".format(self.row_indent, self.row_indent)
        else:
            # For noisy aggregates, infeasible solver instances are treated as underdetermined.
            _step_one_script += "\n{}elif model.Status == 3:".format(self.row_indent)
            _step_one_script += "\n{}{}with open('{}', 'wb') as fp:".format(self.row_indent, self.row_indent, os.path.join(self.save_dir.format(filename), "feasible-solutions-{}_.pkl".format(unique_record_idx)))
            _step_one_script += "\n{}{}{}pickle.dump({{}}, fp)".format(self.row_indent, self.row_indent, self.row_indent)
    ...
    def _attack(self, _input_args):
        ...
        unique_record_query_constraint = {unique_record_query: 1}
        input_constraints = {**released_aggregates, **unique_record_query_constraint}
