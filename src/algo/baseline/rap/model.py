"""
Generative model of synthetic data
Modified the code from https://github.com/terranceliu/dp-query-release/blob/main/src/syndata/generator.py
"""

import torch
from abc import ABC, abstractmethod
from .domain import Dataset, DataTransformer
...
class Generator(ABC):
    ...
    def generate(self):
        ...
        # Each column in one row(record) indicates the probability of each one-hot category
        return x

    def get_answers(self, x, density=False, idxs=None):
        weights = torch.ones(self.K, dtype=torch.float, device=self.device).unsqueeze(-1)
        answers = self.qm.get_answers_helper(x, weights, query_idxs=idxs, batch_size=self.query_bs)
        if density is True:
            answers /= weights.sum()
        return answers

    def get_qm_answers(self, density=False):
        syn = self.generate().detach()
        answers = self.get_answers(syn, density)
        return answers
    ...
    def get_syndata(self, num_samples=100000, how='sample'): 
        samples = []
        ...
        for _ in range(num_samples // self.K):
            syn = self.generate().detach()
...
class NeuralNetworkGenerator(Generator):
    def __init__(self, qm, K=1000, query_bs=10000, agg_mapping={},
    ...
    def get_syndata_by_input(self, input_z, how='sample'):
        x = self.generator(input_z)
        x = self._apply_activate(x)
        for pos, pos_agg_list in self.agg_mapping.items():
            x[:, pos] = x[:, pos_agg_list].sum(axis=-1)
        syn = self._get_onehot(x.detach(), how=how)
        samples = [syn]
        samples = torch.cat(samples, dim=0).cpu()
        samples[:, list(self.agg_mapping.keys())] = 0
        for key, val in self.agg_mapping.items():
