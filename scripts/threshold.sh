...
do
        ...
        do
                python $python_script --dataset ppmf --filenames ${BLOCKS[@]} --seed $seed --num-aggregates $num_aggregates --test-size $test_size --chosen-gpus -1 --multi-cpus 10 --note $note \
                        desia --stochastic-method threshold
        done
done