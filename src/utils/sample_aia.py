...
class AuxiliaryWithoutReplacementSampler():
    ...
    def __init__(self, args, domain_config, target_records: pd.DataFrame, auxiliary_dataset: pd.DataFrame, target_dataset: pd.DataFrame):
        ...
        self.shadow_size = len(self.target_dataset)
        ...
        randomized_auxiliary_dataset_without_target = auxiliary_dataset_without_target.sample(frac = 1.0, random_state = 0)
        ...
        test_indexs = self.target_dataset.index
        merged_df = pd.merge(self.target_dataset.reset_index(), self.target_records[non_sens_attributes], on=non_sens_attributes, how='left', indicator=True).set_index(test_indexs)
        test_dataset_without_target = merged_df[merged_df['_merge'] == 'left_only'].drop(columns=['_merge'])
        self.split['test'] = test_dataset_without_target

    def mask_dataset(self, dataset, idx_target):
        dataset.drop(labels = [self.sens_attribute], axis = 1)
        dataset[self.sens_attribute] = np.random.binomial(n = 1, p = 0.5, size = len(dataset))
        dataset[self.sens_attribute].iloc[idx_target] = np.random.binomial(n = 1, p = 0.5)
        return dataset
    ...
    def sample_dataset(self, seed, eval_type):
        ...
        # Find whether shadow qbs contains repeats of target records
        target_record_not_in_qbs = True
        aux_other_rows = {' '.join(other_record.astype(str)): True for other_record in other_records.to_numpy()}
        for target_record in self.target_records.to_numpy():
            if ' '.join(target_record.astype(str)) in aux_other_rows:
                target_record_not_in_qbs = False
                break
        assert(target_record_not_in_qbs)

        idx_target = np.random.choice(num_other_records + 1, size=1, replace=False)
        list_dfs = []
        start = 0
        for idx, end in enumerate(sorted(idx_target)):
        ...
        dataset = self.mask_dataset(dataset, idx_target)
        ...
        return indexs, dataset[self.sens_attribute].iloc[idx_target]
