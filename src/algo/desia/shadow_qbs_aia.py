...
class StepTwoShadowQbsAttack(BaseAttack):
    ...
    def get_X_train_eval(self, queries, query2answers):
        ...
        if self.args.noise_budget is not None:
            # Mask aggregates with noise in noisy-AIA settings.
            # No unique-record query is part of this feature matrix.
            np.random.seed(self.args.noise_seed)
            noise = np.round(np.random.laplace(0, 1 / self.args.noise_budget, size = X.shape))
            X += noise
    ...
    def _attack(self, _input_args):
        ...
        if domain_config[sens_attribute] == 2:
            ...
            train_log.append("Prediction Area Under ROC curve (OVO) of training shadow datasets: {:.4f}".format(roc_auc_score(y_train, y_train_prob, multi_class="ovo")))
            train_log.append("Prediction Area Under ROC curve (OVO) of validation shadow datasets: {:.4f}".format(roc_auc_score(y_val, y_val_prob, multi_class="ovo")))
            train_log.append("Prediction Area Under ROC curve (OVR) of training shadow datasets: {:.4f}".format(roc_auc_score(y_train, y_train_prob, multi_class="ovr")))
            train_log.append("Prediction Area Under ROC curve (OVR) of validation shadow datasets: {:.4f}".format(roc_auc_score(y_val, y_val_prob, multi_class="ovr")))
    ...
    def _evaluate_shadow_qbses(self, _input_args):
        ...
        if os.path.exists(os.path.join(self.save_dir.format(filename), "feasible-solutions-{}_.pkl".format(unique_record_idx))):
            ...
            aia_shadow_qbses_votes.append(int(clf.predict(X_test) == y_test))
            aia_shadow_qbses_prob.append([round(label_prob, 4) for label_prob in clf.predict_proba(X_test).squeeze().tolist()])
            ...
            aia_shadow_modeling, _ = Counter(aia_shadow_qbses_votes).most_common(1)[0]

            eval_log.append("\nIdx {} / Record: {}".format(unique_record_idx, unique_record.to_dict()))
            eval_log.append("----------Prediction Performance----------")
            eval_log.append("AIA prediction probability by shadow modeling: {}".format(aia_shadow_qbses_prob))
            eval_log.append("AIA prediction votes by shadow modeling: {}".format(aia_shadow_qbses_votes))
            eval_log.append("AIA final prediction by shadow modeling: {}".format("correct" if aia_shadow_modeling == 1 else "wrong"))
